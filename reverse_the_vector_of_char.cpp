...
void solve(vector<char> &vec){
	...
	cout<<"initial character vector"<<nextline;
	for(auto x:vec){
		cout<<x<<space;
	}
	int n=vec.size();
	for(int i=0;i<n/2;i++){
		// cout<<vec[i]<<space<<vec[n-i-1]<<nextline;
		swap(vec[i],vec[n-i-1]);
	}
	cout<<"\n"<<"after reversing"<<nextline;
	for(auto x:vec){
		cout<<x<<space;
	}
}
...
#endif
    // create a vector of characters and pass it to the reverse function to 
    // reverse the string
