...
void solve(){
	...
	// we keep track of max integer encountered so far and max_diff
	int min_ele_so_far=IINF;
	int max_diff_so_far=0;
	loop(0,n){
		min_ele_so_far=min(min_ele_so_far,arr[i]);
		max_diff_so_far=max(max_diff_so_far,arr[i]-min_ele_so_far);
	}
	cout<<max_diff_so_far<<nextline;
